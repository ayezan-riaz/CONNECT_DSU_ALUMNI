...
const NewsPage: React.FC = () => {
  ...
  const handleDeleteConfirmation = () => {
    if (selectedNews) {
      axios.delete(`https://ams-backend-gkxg.onrender.com/api/news/${selectedNews.id}`)
        .then(() => {
  ...
  return (
    <div>
          ...
          <div key={newss.id} className="col-md-4">
            <div className="card-xl-stretch me-md-6">
              <span style={{ textAlign: 'right' }}>
                <i className="fa fa-times-circle" style={{ fontSize: '20px', color: '#80171d', cursor: 'pointer' }} onClick={() => handleDelete(newss)}></i>
              </span>
              ...
              <div className="m-0">
                ...
                <Link to={`/alumni/dsu/newsDetail/${newss.id}`}
                ...
                <span style={{ marginLeft: '10px', cursor: 'pointer' }} onClick={() => openModal(newss)}>
                ...
                <div className="fs-6 fw-bold">
                  <span className="text-muted">
                    {new Date(newss.date).toDateString()}
                  </span>
                </div>
              </div>
            </div>
      ...
      <NewsModal isOpen={showModal} onClose={closeModal} selectedNews={selectedNews} fetchNews={fetchNews} />
